...
- 拉伸侧同步补 CLREQ 限制：`AvoidStretchAroundConnectors`——均匀拉大
  字距不触碰连接号/分隔号（Connector/Solidus）前后的边界。2026-06-25
  追加 `NoStretchBoundaryClusters` 命名，覆盖同一 CLREQ 限制和项目的
  原子长标号模型（Dash/Ellipsis 前后也不拉伸），避免 justify 制造
  `—— 不` 式假空格；数字+单位等未建模项随该特性落地时一并处理。

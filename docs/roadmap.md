...
```text
Last completed: Slice 14 (段首缩进，gap audit 缺口 5，ADR 0021)
Up next:        Slice 15 (gap audit 缺口 7：行间线——专名号/书名号甲式，复用 decoration 通道) 或缺口 6（行尾悬挂 opt-in）；更远期：Android composable 渲染、竖排预研
```

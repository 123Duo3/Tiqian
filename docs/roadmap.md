...
| 3 | M2 | PunctuationAtom（ink/body/leadingGlue/trailingGlue）；行尾标点自然半宽；连续标点挤压；引号 / 括号成对感知 | `中文，中文。` `他说：“你好，世界。”` `中文……中文。` `中文(English)中文` | dump `punct:*` 和 `spacing:*` 行；`QuotePairAnalyzerTest` `BracketPairAnalyzerTest` | wip (`PunctuationAtomBuilder` / spacing compression / quote pair / bracket pair 都 done；剩余 `inkBounds` 等 Slice 6 真实 shaping) |
